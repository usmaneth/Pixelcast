				...
				<div className={`${styles.electronDrag} ${styles.hudBarWrapper}`}>
					...
					>
						...
						<Button
							variant="link"
							size="sm"
							className={`gap-1.5 text-[#EEEEF0] bg-transparent px-2.5 py-1.5 text-sm rounded-xl ${styles.electronNoDrag} ${styles.hudBtn}`}
							onClick={openSourceSelector}
							disabled={recording}
							title={selectedSource}
						>
							<MdMonitor size={17} className="text-[#A1A1A6]" />
						...
						<div className={`flex items-center gap-1.5 ${styles.electronNoDrag}`}>
							{supportsHudCaptureProtection && (
								<Button
									...
									className={`text-[#EEEEF0] rounded-xl ${styles.hudBtn} ${
										!hideHudFromCapture ? styles.toggleBtnActive : ""
									}`}
								>
									{hideHudFromCapture ? (
										<EyeOff size={18} className="text-[#666666]" />
									) : (
							...
							>
								{systemAudioEnabled ? (
									<MdVolumeUp size={18} className="text-[#0A84FF]" />
								) : (
									<MdVolumeOff size={18} className="text-[#666666]" />
								)}
							</Button>
							<Button
								variant="link"
							...
							>
								{microphoneEnabled ? (
									<MdMic size={18} className="text-[#0A84FF]" />
								) : (
									<MdMicOff size={18} className="text-[#666666]" />
								)}
							</Button>
							<Button
								variant="link"
							...
							>
								{cameraEnabled ? (
									<Camera size={18} className="text-[#30D158]" />
								) : (
									<CameraOff size={18} className="text-[#666666]" />
								)}
							</Button>
						</div>
							...
							<DropdownMenu>
								<DropdownMenuTrigger asChild>
									<Button
										...
										className={`gap-1.5 px-2 py-1.5 text-sm text-[#A1A1A6] rounded-xl ${styles.electronNoDrag} ${styles.hudBtn}`}
									>
										<Camera size={16} />
							...
							<DropdownMenu>
								<DropdownMenuTrigger asChild>
									<Button
										...
										size="sm"
										title="Camera shape"
										className={`px-2 py-1.5 text-xs text-[#A1A1A6] rounded-xl ${styles.electronNoDrag} ${styles.hudBtn}`}
									>
										{webcamShape === "circle"
											? "\u25CF"
								...
								>
									<DropdownMenuItem
										...
										className={`cursor-pointer text-sm py-2 px-3 ${styles.dropdownItem} ${webcamShape === "circle" ? "font-medium text-white" : "text-[#A1A1A6]"}`}
									>
										Circle
									</DropdownMenuItem>
									<DropdownMenuItem
										onSelect={() => setWebcamShape("rounded-square")}
										className={`cursor-pointer text-sm py-2 px-3 ${styles.dropdownItem} ${webcamShape === "rounded-square" ? "font-medium text-white" : "text-[#A1A1A6]"}`}
									>
										Rounded
									</DropdownMenuItem>
									<DropdownMenuItem
										onSelect={() => setWebcamShape("square")}
										className={`cursor-pointer text-sm py-2 px-3 ${styles.dropdownItem} ${webcamShape === "square" ? "font-medium text-white" : "text-[#A1A1A6]"}`}
									>
										Square
									</DropdownMenuItem>
								</DropdownMenuContent>
							...
							>
								{[0, 3, 5, 10].map((delay) => (
									<DropdownMenuItem
										...
										className={`cursor-pointer text-sm py-2 px-3 ${styles.dropdownItem} ${
											countdownDelay === delay ? "font-medium text-white" : "text-[#A1A1A6]"
										}`}
									>
						...
						>
							{recording ? (
								<>
									<FaRegStopCircle size={18} className={`text-[#FF453A] ${styles.pulseRecord}`} />
									<span className="text-[#FF453A] font-bold tabular-nums ">
										{formatTime(elapsed)}
									</span>
								</>
							) : starting ? (
								<>
									<Loader2 size={18} className="animate-spin text-[#A1A1A6]" />
									<span className="text-[#A1A1A6] font-medium tracking-wide">
								...
								<>
									<BsRecordCircle
										size={18}
										className={hasSelectedSource ? "text-white " : "text-[#666666]"}
									/>
									<span
										className={
											hasSelectedSource ? "text-white font-medium tracking-wide " : "text-[#666666]"
										}
									>
										{t("recording.record")}
									</span>
						...
						<Button
							...
							className={`text-[#A1A1A6] px-2 py-1.5 ml-1 text-sm underline decoration-white/30 underline-offset-4 rounded-xl ${styles.electronNoDrag} ${styles.hudBtn}`}
						>
						...
						<div className="ml-auto flex items-center gap-1.5">
							...
							<Button
								...
								className={`text-[#A1A1A6] rounded-xl ${styles.electronNoDrag} ${styles.hudBtn}`}
							>
								<MdVideoFile size={18} />
							</Button>
							<Button
									...
									{SUPPORTED_LOCALES.map((code) => (
										<DropdownMenuItem
											...
											className={`text-sm py-2 px-3 cursor-pointer ${styles.dropdownItem} ${
												locale === code ? "text-white font-medium" : "text-[#A1A1A6]"
											}`}
										>
